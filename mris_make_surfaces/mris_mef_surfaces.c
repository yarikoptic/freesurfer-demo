...
static char vcid[] = "$Id: mris_mef_surfaces.c,v 1.1.2.1 2007/01/09 19:15:32 nicks Exp $";
...
main(int argc, char *argv[])
{
  ...
  make_cmd_version_string (argc, argv, "$Id: mris_mef_surfaces.c,v 1.1.2.1 2007/01/09 19:15:32 nicks Exp $", "$Name:  $", cmdline);

  /* rkt: check for and handle version tag */
  nargs = handle_version_option (argc, argv, "$Id: mris_mef_surfaces.c,v 1.1.2.1 2007/01/09 19:15:32 nicks Exp $", "$Name:  $");
  ...
  for (j = 0 ; j <= 0 ; parms.l_intensity *= 2, j++)  /* only once for now */
  {
         ...
         n_averages /= 2, current_sigma /= 2, i++)
    {
      ...
      */
      ...
      MRIScomputeBorderValues_MEF_PIAL(mris, mri_em_seg, mri_T1_30, mri_T1_5, 
				  ...
				  float max_thickness, FILE *log_fp)
{
  ...
  for (total_vertices = vno = 0 ; vno < mris->nvertices ; vno++)
      ...
      for (dist = inward_dist ; dist <= outward_dist ; dist += STEP_SIZE)
	{
	  ...
	  if (previous_val30 < (wm_mean[0] + 3*wm_std[0]) && previous_val30 > gm_mean[0])
	    {
		...
		and it is in the right intensity range....
	      */
		  ...
		  (val30 <= (wm_mean[0]+wm_std[0])) && (val30 >= gm_mean[0]) //is this too restrictive??
		  )
		{
		  x = v->x + v->nx*(dist+1) ;
		  y = v->y + v->ny*(dist+1) ;
      ...
      for (current_sigma = sigma; current_sigma <= 10*sigma; current_sigma *= 2)
	{
	  for (dist = 0 ; dist > -max_thickness ; dist -= step_size)
		...
		break ;
	      ...
	      if (val5 > (gm_mean[1] - 0.5*gm_std[1]) || val30 > (wm_mean[0] - 0.5*wm_std[0]))
		break ;
	    }
	  inward_dist = dist+step_size/2 ; 
	  for (dist = 0 ; dist < max_thickness ; dist += step_size)
