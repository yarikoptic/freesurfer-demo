...
const char *MRI_WATERSHED_VERSION = "$Revision: 1.81.2.2 $";
...
static void analyseCSF(unsigned long CSF_percent[6][256],
                       MRI_variables *MRI_var, 
                       int i);
static void analyseGM(unsigned long CSF_percent[6][256],
...
void MRI_weight_atlas(MRI *mri_with_skull, 
                      TRANSFORM *transform, 
                      STRIP_PARMS *parms);
static void FindSeedPrior(STRIP_PARMS *parms,
                          MRI_variables *MRI_var);
...
void usageHelp()
{
  ...
  fprintf(stdout,
          "The default parameters are: -w 0.82 -b 0.32 -h 10 "
          "-seedpt -ta -wta "
          "\n                       "
          "You can use one of the five following flags to change\n"
          "                       these default parms :  ");
  fprintf(stdout, "\n-w weight            : "
  ...
  fprintf(stdout,
          "                                               "
          "Note: this does not change output,\n"
          "                                               "
          "just creates NewWithEditsVol\n");

  fprintf(stdout, 
          "\n-brain_atlas atlas.gca register.lta : "
          "use the registration file (register.lta) to take advantage\n"
          "                         "
          "             of atlas (atlas.gca) information ");
...
int main(int argc, char *argv[])
{
  ...
  make_cmd_version_string
    (argc, argv,
     "$Id: mri_watershed.cpp,v 1.81.2.2 2010/11/11 20:31:14 greve Exp $", 
     "$Name:  $",
     cmdline);
...
/*-----------------------------------------------------
  ...
  Description:
  ------------------------------------------------------*/
static void AnalyzeCerebellum(STRIP_PARMS *parms,
                              MRI_variables *MRI_var, 
                              float & mean, 
                              float &var)
{
...
/*-----------------------------------------------------
  ...
  ------------------------------------------------------*/
static void BASIN_PRIOR(STRIP_PARMS *parms,MRI_variables *MRI_var)
{
  int i=0,j=0,k;
  ...
  for (k=2;k<MRI_var->depth-2;k++)
  {
    for (j=2;j<MRI_var->height-2;j++)
    {
      for (i=2;i<MRI_var->width-2;i++)
      {
        ...
        if (cell->type==1)  cell1=TypeVoxel(cell);
        else cell1=cell;
        if (cell->type)
...
static void FitShape(MRI_variables *MRI_var,  STRIP_PARMS *parms,
